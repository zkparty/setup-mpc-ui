...
export const ParticipantSection = observer(() => {
  ...
  const { authenticated, inQueue, step, computeStatus, participant,  
     isFinished, userContributions, summaryGistUrl, project, accessToken } = ceremony;

  useEffect(() => {
    handleStepChange(ceremony);
  },[step ]);
  ...
  //   if (isFinished && userContributions && userContributions.length>0 && !summaryGistUrl && !summaryStarted.current) {
  ...
  //   }
  // },
  // [isFinished, userContributions?.length, summaryGistUrl, summaryStarted.current, accessToken, dispatch]
  // );
